...
static uint64_t find_period(uint64_t N, std::ostream& os)
{
    ...
    OTI::xorshift4096star rng;
    const OTI::xorshift4096star::seed_type* the_seed = rng.seed();
    ...
    for( uint64_t i = 0; i != N; ++i )
    {
        ...
        {
            rng.sample();
            if (std::equal( the_seed,  the_seed + 64, rng.seed()))
            {
...
static bool test_skip_ahead(int64_t ns)
{
    ...
    return std::equal(rng.seed(), rng.seed() + 64, rngs.seed());
}
...
static bool test_skip_zero()
{
    ...
    return std::equal(the_seed, the_seed + 64, rng.seed());
}


int main(int argc, char* argv[])
{
    OTI::xorshift4096star::result_type period = find_period(2500000000ULL, std::cout); 
